...
using System.Text;
// using GardenConquest.Autopilot.Data;
        ...
        */
		...
		private static void createLog()
		{
			try
			{
				if (MyAPIGateway.Utilities.FileExistsInLocalStorage(s_logMaster, typeof(Logger)))
				{
                    ...
                    Rynchodon.Utility.FileMaster master = new Rynchodon.Utility.FileMaster(s_logMaster, "log-", 10);
		...
		public static void AppendStack(StringBuilder builder, StackTrace stackTrace, params Type[] skipTypes)
		{
			...
			while (frame < totalFrames)
			{
				...
				if (!method.DeclaringType.Namespace.StartsWith("GardenConquest"))
					break;
				appendedFrame = true;
				builder.Append("   at ");
