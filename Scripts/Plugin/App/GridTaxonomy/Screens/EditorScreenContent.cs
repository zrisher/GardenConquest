...
{
	class EditorScreen : MyGuiScreenScenarioBase
	{
		...
		protected override void LoadSandboxInternal(Tuple<string, MyWorldInfo> save, bool MP)
		{
			...
			if (save.Item1 == WORKSHOP_PATH_TAG)
			{
				...
				MySteamWorkshop.CreateWorldInstanceAsync(scenario, MySteamWorkshop.MyWorkshopPathInfo.CreateScenarioInfo(), true, delegate (bool success, string sessionPath)
				{
					if (success)
					{
						...
						MyScenarioSystem.LoadMission(sessionPath, //m_nameTextbox.Text, m_descriptionTextbox.Text,
						MP, (MyOnlineModeEnum)m_onlineMode.GetSelectedKey(), (short)m_maxPlayersSlider.Value);
					}
					else
			...
			{
				MyAnalyticsHelper.SetEntry(MyGameEntryEnum.Scenario);
				MyScenarioSystem.LoadMission(save.Item1, //m_nameTextbox.Text, m_descriptionTextbox.Text,
				MP, (MyOnlineModeEnum)m_onlineMode.GetSelectedKey(), (short)m_maxPlayersSlider.Value);
			}
		}
				...
				callback: delegate (MyGuiScreenMessageBox.ResultEnum val)
				{
					if (val == MyGuiScreenMessageBox.ResultEnum.YES)
					{
                        ...
                        //{
                            ...
                            //{
						MySteamWorkshop.PublishScenarioAsync(fullPath, worldInfo.SessionName, worldInfo.Description, worldInfo.WorkshopId, //outTags,
						SteamSDK.PublishedFileVisibility.Public, callbackOnFinished: delegate (bool success, Result result, ulong publishedFileId)//TODO public visibility!!
						{
							if (success)
							{
								ulong dummy;
