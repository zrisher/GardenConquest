...
// using GardenConquest.AntennaRelay;
// using GardenConquest.Attached;
// using GardenConquest.Autopilot;
// using GardenConquest.Autopilot.Aerodynamics;
// using GardenConquest.Autopilot.Harvest;
using Rynchodon;
using Rynchodon.Settings;
using Rynchodon.Threading;
using Rynchodon.Utility.Collections;
// using GardenConquest.Weapons;
// using GardenConquest.Weapons.Guided;
// using GardenConquest.Weapons.SystemDisruption;
	...
	public class UpdateManager : MySessionComponentBase
	{
				...
				foreach (KeyValuePair<uint, List<Action>> pair in UpdateRegistrar)
					if (Globals.UpdateCount % pair.Key == 0)
					{
						foreach (Action item in pair.Value)
							try
							{
                                Rynchodon.Utility.Profiler.StartProfileBlock(item);
								item.Invoke();
                                Rynchodon.Utility.Profiler.EndProfileBlock();
							}
							catch (Exception ex2)
							{
								if (Unregister == null)
		...
		protected override void UnloadData()
		{
			...
			if (!Globals.WorldClosed)
			{
                ...
                Rynchodon.Utility.Profiler.Write();
			}
